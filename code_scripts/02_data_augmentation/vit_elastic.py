...
# Dataset class with uniform transformation
class RAFDBDataset(Dataset):
    def __init__(self, img_dir, label_df, transform=None, advanced_transform=None):
    ...
    def __getitem__(self, idx):
        ...
        # Apply advanced_transform if defined; otherwise, fallback to transform
        if self.advanced_transform:
            image = self.advanced_transform(image)
        elif self.transform:
...
def elastic_transform(image, alpha=50, sigma=6, random_state=None):
...
for run in range(num_runs):
    ...
    train_dataset = RAFDBDataset(
        img_dir, label_df[label_df['image_name'].str.startswith('train')],
        advanced_transform=lambda image: elastic_transform(image, alpha=50, sigma=6)
    )
    test_dataset = RAFDBDataset(
        img_dir, label_df[label_df['image_name'].str.startswith('test')], transform=general_transform
    )
...
final_results_df.to_csv('./vit_elastic.csv', index=False)

print("Predictions with metadata for all datasets saved to './vit_elastic.csv'")
