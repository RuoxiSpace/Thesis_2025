...
def initialize_model(num_classes=7):
    ...
    # Prune Multi-Attention Heads
    for block in model.blocks:
        block.attn.num_heads = 6  # from 12 to 6
        block.attn.head_dim = block.attn.qkv.weight.shape[-1] // block.attn.num_heads
        block.attn.scale = 1 / (block.attn.head_dim ** 0.5)
    return model
...
final_results_df.to_csv('./t4.csv', index=False)

print("Predictions with metadata saved to './t4.csv'")
