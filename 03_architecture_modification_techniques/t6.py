...
def initialize_model(num_classes=7):
    ...
    # Modify Input Size
    new_img_size = 100
    model.patch_embed = PatchEmbed(img_size=new_img_size, patch_size=16, in_chans=3, embed_dim=768)
    model.patch_embed.grid_size = (new_img_size // 16, new_img_size // 16)  # Update grid size

    # Adjust Positional Embeddings
    num_patches = (new_img_size // 16) ** 2  # Total number of patches
    pos_embed = model.pos_embed[:, :(1 + num_patches), :]  # Resize positional embeddings
    if pos_embed.size(1) != 1 + num_patches:
        raise ValueError("Mismatch in positional embedding size. Check grid size and patch size.")

    model.pos_embed = nn.Parameter(pos_embed)  # Update positional embeddings
    model.head = nn.Linear(model.head.in_features, num_classes)  # Update classification head
...
final_results_df.to_csv('./t6.csv', index=False)

print("Predictions with metadata saved to './t6.csv'")
