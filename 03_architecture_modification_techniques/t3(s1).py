...
def initialize_model(num_classes=7):
    ...
    # Modify the Transformer Encoder with 6 layers
    model.blocks = model.blocks[:6]  # Keep only the first 6 layers
...
final_results_df.to_csv('./t3(s1).csv', index=False)

print("Predictions with metadata saved to './t3(s1).csv'")
