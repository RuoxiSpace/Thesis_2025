...
def initialize_model(num_classes=7):
    ...
    # Modify the Transformer Encoder with 10 layers
    model.blocks = model.blocks[:10]  # Keep only the first 10 layers
...
final_results_df.to_csv('./s4.csv', index=False)

print("Predictions with metadata saved to './s4.csv'")
