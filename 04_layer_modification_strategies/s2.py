...
def initialize_model(num_classes=7):
    ...
    # Modify the Transformer Encoder with 8 layers
    model.blocks = model.blocks[:8]  # Keep only the first 8 layers
...
final_results_df.to_csv('./s2.csv', index=False)

print("Predictions with metadata saved to './s2.csv'")
